...
Warrior placeholder di chuyển WASD, có hướng/state, giới hạn map và camera follow.

## Đầu vào và phụ thuộc

- [Phase 04 — Client bootstrap](phase-04-client-bootstrap.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Dùng input/góc nhìn đã chốt ở GDD; camera và vận tốc ban đầu ghi trong cấu hình dễ chỉnh.

## Các bước thực hiện

1. Tách player khỏi scene; ánh xạ WASD thành hướng và vận tốc theo thời gian.
2. Giới hạn world bounds, camera follow và cập nhật IDLE/MOVING cùng hướng cuối.
3. Chuẩn hóa di chuyển chéo; ngừng input khi mất focus để tránh nhân vật chạy kẹt.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-client/src/entities/`; `game-client/src/scenes/`; `thành phần movement phù hợp`.
...
Attack, skill, animation đầy đủ, enemy hoặc network.
...
- [ ] Nhấn/nhả phím đổi chuyển động đúng, đứng lại khi nhả hết phím.
- [ ] Tốc độ chéo không cao hơn tốc độ thẳng và không phụ thuộc FPS render.
- [ ] Player không vượt biên; camera theo đúng giới hạn map.
...
Thử bốn hướng, hai phím chéo, phím đối nhau, góc map và chuyển tab; chạy build/typecheck của client.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-05-player-movement.md. Thực hiện chỉ Phase 5 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 5, chưa triển khai code.”**
