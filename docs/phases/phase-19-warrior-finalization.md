...
Combo/burst và tương tác rõ với trạng thái stagger.
...
**Điểm cần xử lý trước phần phụ thuộc:** Không thay balance các class khác để làm Warrior mạnh hơn trong phạm vi phase này.

## Các bước thực hiện

1. Đưa Warrior lên kiến trúc class mới với bộ MVP đã chọn; thêm combo/burst trong phạm vi đó.
2. Nối hệ số/điều kiện tấn công core trong cửa sổ stagger theo thiết kế combat.
3. Làm rõ vào/ra đòn, cancel và di chuyển áp sát; giữ phản hồi đánh trúng dễ quan sát.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-client/src/skills/`; `cấu hình Warrior`; `tương tác stagger/core`.
...
Bắt buộc làm toàn bộ Slash/Leap/Armor Break/Execute nếu chưa chọn, thêm gear hoặc network.
...
- [ ] Warrior có lợi thế rõ khi tấn công trong burst window theo quy tắc đã chọn.
- [ ] Bonus không tồn tại sau khi stagger/core window đóng.
- [ ] Combo/cancel và cooldown không cho spam damage hoặc đánh sau khi chết.
...
So sánh cùng đòn trước/trong/sau stagger; thử cancel, hụt đòn và boss chết trong combo.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-19-warrior-finalization.md. Thực hiện chỉ Phase 19 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 19, chưa triển khai code.”**
