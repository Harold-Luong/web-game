...
Mọi thay đổi HP do tấn công đi qua pipeline chung, có sự kiện damage/death.

## Đầu vào và phụ thuộc

- [Phase 01 — Combat design](phase-01-combat-design.md).
- [Phase 09 — Hitbox/hurtbox](phase-09-hitbox-hurtbox.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Dùng chính xác quy tắc làm tròn, shield và multi-hit từ Phase 1; ghi rõ nếu thiết kế còn thiếu.

## Các bước thực hiện

1. Thiết lập DamageRequest, phép tính và DamageResult theo COMBAT.
2. Áp dụng shield/defense/state, giới hạn HP; phát damage và death event ở một điểm thống nhất.
3. Nối hit từ Phase 9 vào pipeline; cung cấp feedback tối thiểu để quan sát kết quả.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-client/src/combat/`; `trạng thái HP và combat events`.
...
Crit/resistance chưa thiết kế, loot, backend hoặc cân bằng đầy đủ.
...
- [ ] Ví dụ số trong COMBAT cho kết quả đúng, HP nằm trong miền hợp lệ.
- [ ] Một lần chết chỉ phát death event một lần; mục tiêu chết/invulnerable xử lý đúng luật.
- [ ] Không có đường input/scene tự trừ HP bỏ qua pipeline.
...
Kiểm tra damage tối thiểu, defense cao, shield, invulnerable, overkill và hai hit cùng thời điểm; chạy thử đánh dummy.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-10-damage-system.md. Thực hiện chỉ Phase 10 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 10, chưa triển khai code.”**
