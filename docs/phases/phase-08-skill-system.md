...
Skill definition, trạng thái thi triển và cooldown hoạt động độc lập với input.

## Đầu vào và phụ thuộc

- [Phase 01 — Combat design](phase-01-combat-design.md).
- [Phase 02 — Class design](phase-02-class-design.md).
- [Phase 07 — Entity architecture](phase-07-entity-architecture.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Nếu mana/passive chưa được chọn ở Phase 2 thì không tự thêm hệ tài nguyên hoặc cây kỹ năng.

## Các bước thực hiện

1. Tách SkillDefinition khỏi trạng thái từng lần cast và cooldown của từng entity.
2. Ánh xạ input vào yêu cầu cast; kiểm tra state, cooldown, resource đã thiết kế và xử lý cancel.
3. Đưa Warrior basic + skill MVP qua hệ thống; phát thông tin thời gian hiệu lực để Phase 9 nối hitbox.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-client/src/skills/`; `cấu hình skill và cooldown`.
...
Trừ HP trực tiếp, toàn bộ skill bốn class, progression/unlock bền vững.
...
- [ ] Giữ hoặc spam phím không bỏ qua cooldown; hai entity không dùng chung cooldown ngoài ý muốn.
- [ ] Nhân vật chết hoặc ở state cấm không thể cast.
- [ ] Cast/cancel/recast tuân theo thiết kế và có điểm tích hợp cho animation/hitbox.
...
Kiểm tra bằng đồng hồ mô phỏng ở trước/đúng/sau thời điểm hết cooldown và khi cast bị ngắt; xác minh input trong browser.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-08-skill-system.md. Thực hiện chỉ Phase 8 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 8, chưa triển khai code.”**
