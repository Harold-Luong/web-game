...
Create/join/leave, chọn class, ready/start; bốn browser vào cùng room.

## Đầu vào và phụ thuộc

- [Phase 27 — Network protocol](phase-27-network-protocol.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Dùng luật đủ người/trùng class từ GDD; nếu chưa chốt phải giải quyết trước phần phụ thuộc.

## Các bước thực hiện

1. Implement create/join/leave, chọn class, ready/start với state room theo thiết kế.
2. Cấp ID người chơi tạm gắn với kết nối, kiểm tra sức chứa/đội hình và quyền thực hiện lệnh.
3. Quản lý room hết người, nhiều join đồng thời, reset ready khi đội hình thay đổi và broadcast room state.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-server/ — room/player/network`; `UI lobby tối thiểu phía client`.
...
Combat server, matchmaking tự động, tài khoản/DB hoặc giao diện lobby hoàn thiện.
...
- [ ] Bốn browser vào cùng room với bốn class và chỉ start khi đủ điều kiện.
- [ ] Người thứ năm, class trùng hoặc lệnh sai state bị xử lý theo luật đã chốt.
- [ ] Room độc lập; người ngoài không sửa trạng thái room khác; leave không để slot ma.
...
Thử bốn browser, join đồng thời, leave khi ready, start lặp và hai room riêng; kiểm tra room cleanup.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-28-room-system.md. Thực hiện chỉ Phase 28 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 28, chưa triển khai code.”**
