...
Schema và truy cập dữ liệu bền vững; không ghi trạng thái realtime từng tick.
...
**Điểm cần xử lý trước phần phụ thuộc:** Chọn quy tắc định danh/unique và công cụ migration khi triển khai; không hard-code credential vào repository.

## Các bước thực hiện

1. Thiết kế schema tối thiểu cho user/character/progress cùng ràng buộc sở hữu; item/inventory sẽ mở rộng theo phase.
2. Thêm PostgreSQL/JPA và migration, nối tài khoản thử nghiệm sang persistence thật với mật khẩu được băm.
3. Tách truy cập DB khỏi realtime tick, cấu hình credential qua môi trường và ghi cách setup.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-server/ — persistence và migration`; `docs/data/SCHEMA.md`; `cấu hình DB theo môi trường`.
...
Inventory/equipment đầy đủ, Redis hoặc mọi bảng tương lai chưa dùng.
...
- [ ] Database mới được dựng bằng migration có thứ tự và chạy lại không phá dữ liệu.
- [ ] Tài khoản tồn tại sau restart; ràng buộc unique/foreign key theo schema được bảo đảm.
- [ ] Không có thao tác lưu position/projectile/HP enemy mỗi tick.
...
Chạy migration trên DB trống và DB đã có, thử tạo username trùng, restart auth và kiểm tra giao dịch lỗi không ghi nửa chừng.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-38-postgresql.md. Thực hiện chỉ Phase 38 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 38, chưa triển khai code.”**
