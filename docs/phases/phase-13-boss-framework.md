...
**Điểm cần xử lý trước phần phụ thuộc:** Triển khai phần state thực sự cần; tên state chưa sử dụng có thể giữ ở thiết kế thay vì code rỗng.

## Các bước thực hiện

1. Thiết lập state/phase boss với entry/exit, lịch tấn công và cancellation rõ ràng.
2. Tạo pattern đơn giản có telegraph → delay → hit theo combat pipeline.
3. Hiển thị HP/state tối thiểu và nối boss chết với encounter; tạo điểm mở rộng shield/stagger.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-client/src/bosses/`; `state machine và boss HP UI tối thiểu`.
...
Full Golem bốn class, nhiều boss hoặc framework tổng quát cho mọi thể loại.
...
- [ ] Boss chuyển state theo trigger đã định và không chạy hai đòn trái luật cùng lúc.
- [ ] Telegraph xảy ra trước vùng gây hit; boss chết hủy đòn đang chờ.
- [ ] Restart encounter trả state/timer về ban đầu.
...
Kích hoạt từng chuyển state, giết boss trong thời gian telegraph và restart nhiều lần; kiểm tra pipeline damage không bị bỏ qua.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-13-boss-framework.md. Thực hiện chỉ Phase 13 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 13, chưa triển khai code.”**
