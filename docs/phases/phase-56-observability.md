...
Log và metric cho room/player, tick time, latency, DB và lỗi.

## Đầu vào và phụ thuộc

- [Phase 31 — Server game loop](phase-31-server-game-loop.md).
- [Phase 38 — PostgreSQL](phase-38-postgresql.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Chọn công cụ theo môi trường đã có; có thể bắt đầu log/metric tối thiểu, không bắt buộc một nhà cung cấp cụ thể.

## Các bước thực hiện

1. Chọn log và metric cho room/player count, tick duration, command queue, WebSocket/DB latency và error rate.
2. Gắn correlation hợp lý để theo dõi từ phiên/room tới lỗi, kiểm soát cardinality và dữ liệu nhạy cảm.
3. Viết cách đọc số liệu, cảnh báo theo ngưỡng có cơ sở và hướng điều tra sự cố thường gặp.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `server logs/metrics/error tracking`; `docs/operations/OBSERVABILITY.md`.
...
Triển khai mọi dịch vụ monitoring trả phí, PII analytics hoặc alert chưa có người/đích xử lý.
...
- [ ] Có thể truy vết lỗi một room mà không cần ghi mọi tick hoặc token.
- [ ] Metric active room/player phản ánh đúng sau join/leave/cleanup.
- [ ] Lỗi giả lập và tick chậm xuất hiện ở kênh quan sát dự kiến, không làm nghẽn game loop.
...
Tạo room, cho người vào/rời, gây lỗi có kiểm soát và DB chậm; đối chiếu metric với state thực tế và rà log credential.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-56-observability.md. Thực hiện chỉ Phase 56 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 56, chưa triển khai code.”**
