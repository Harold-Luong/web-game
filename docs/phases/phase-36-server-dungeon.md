...
Room clear, cửa, wave, boss, thắng/thua do server quyết định.
...
**Điểm cần xử lý trước phần phụ thuộc:** Bản multiplayer MVP kết thúc ở kết quả trận; phần thưởng có thể chỉ là kết quả minh họa đến Phase 40.

## Các bước thực hiện

1. Chuyển room clear, cửa, spawn wave, checkpoint và trạng thái dungeon sang server.
2. Ghép lobby/start → dungeon → boss → thắng/thua và thông báo kết quả cho cả đội.
3. Hủy mô phỏng/cleanup theo luật khi kết thúc, retry hoặc thiếu người; xử lý event trùng.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-server/ — dungeon/room/wave/results`; `client scene transitions`.
...
Auth tài khoản, DB, loot/inventory bền vững hoặc scaling.
...
- [ ] Bốn người hoàn thành được cùng một dungeon với kết quả nhất quán.
- [ ] Client không tự mở cửa, clear room hoặc khai chiến thắng ngoài state server.
- [ ] Disconnect một người không crash server; room khác và lượt chơi sau không mang state cũ.
...
Chạy bốn browser thắng/thua/retry, disconnect ở boss death và hai room đồng thời; ghi bằng chứng gate M6.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-36-server-dungeon.md. Thực hiện chỉ Phase 36 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 36, chưa triển khai code.”**
