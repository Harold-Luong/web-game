# Phase 29 — Player synchronization
...
Các client thấy movement/direction/state của nhau với interpolation.

## Đầu vào và phụ thuộc

- [Phase 28 — Room system](phase-28-room-system.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Ghi rõ phần state tạm tin client trong prototype; server-authoritative movement hoàn chỉnh ở Phase 32.

## Các bước thực hiện

1. Truyền input/state cần cho movement, direction và animation qua protocol đã chốt.
2. Tạo buffer snapshot và interpolation cho remote entity; xử lý spawn/despawn theo ID.
3. Giới hạn đồng bộ trong room, xử lý snapshot cũ và reset buffer khi chuyển room.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-client/src/network/`; `remote player presentation`; `server player state`.
...
Combat authoritative, AI server hoặc tuyên bố chống cheat movement trước Phase 32.
...
- [ ] Bốn client thấy nhau di chuyển và đổi hướng tương đối liên tục trong cùng room.
- [ ] Remote entity không nhân đôi khi nhận lại snapshot/spawn.
- [ ] Người chơi ở room khác không xuất hiện; leave/chuyển room dọn state cũ.
...
Quan sát bốn client di chuyển chéo nhau, dừng/đổi hướng và rời room; mô phỏng trễ/jitter ở mức công cụ hỗ trợ.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-29-player-synchronization.md. Thực hiện chỉ Phase 29 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 29, chưa triển khai code.”**
