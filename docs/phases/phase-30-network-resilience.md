# Phase 30 — Network resilience

[README dự án](../../README.md) · [Hướng dẫn agent](../../AGENTS.md)

| Thuộc tính | Giá trị |
| --- | --- |
| Nhóm | E — Nền tảng multiplayer |
...
Heartbeat, phát hiện disconnect, sequence/timestamp và nền tảng reconnect.

## Đầu vào và phụ thuộc

- [Phase 29 — Player synchronization](phase-29-player-synchronization.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Timeout, thời gian giữ slot và quyền reconnect phải được ghi trong protocol; chưa có tài khoản thì token chỉ định danh phiên tạm.

## Các bước thực hiện

1. Bổ sung heartbeat, timeout và phát hiện mất kết nối bằng đồng hồ phù hợp.
2. Xử lý sequence trùng/cũ, reconnect attempt có giới hạn và session token tạm gắn đúng người.
3. Dọn state khi rời room thật sự, phân biệt kết nối gián đoạn với phiên kết thúc; ghi giới hạn phục hồi hiện tại.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `client/server network lifecycle`; `protocol reconnect nền tảng`.
...
Khôi phục sau server restart, Redis session, AI thay người và full reconnect gameplay Phase 48.
...
Server đầu tiên dùng state trong bộ nhớ. Định danh phiên tạm không phải tài khoản bền vững; chưa coi đồng bộ movement là gameplay authoritative hoàn chỉnh.
...
- [ ] Đóng tab hoặc mất mạng được phát hiện trong timeout đã cấu hình.
- [ ] Reconnect không tạo hai player cho cùng một phiên và không nhận phiên của người khác.
- [ ] Packet cũ/trùng không làm lặp hành động đã xác nhận; room không crash vì disconnect.
...
Ngắt mạng/đóng tab, reconnect nhiều lần, gửi sequence trùng và token sai; kiểm tra listener/timer không nhân đôi.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-30-network-resilience.md. Thực hiện chỉ Phase 30 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 30, chưa triển khai code.”**
