...
Shield, weakpoint, aggro, phase, stagger và enrage thống nhất giữa các client.

## Đầu vào và phụ thuộc

- [Phase 24 — Full Golem](phase-24-full-golem.md).
- [Phase 34 — Server enemy AI](phase-34-server-enemy-ai.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Thời gian telegraph phải truyền theo contract để client trễ vẫn hiểu trạng thái; không phụ thuộc callback animation client.

## Các bước thực hiện

1. Chuyển Golem/mini boss vào server: target, shield, crystal, stagger, phase và pattern.
2. Phát mốc thời gian telegraph/cast và state đủ cho client trình bày nhất quán.
3. Xử lý nhiều hit cùng tick, threshold vượt nhiều phase, disconnect và chết khi đang thi triển.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-server/ — boss state/phase/weakpoint`; `client boss presentation`.
...
Boss mới, client tự quyết stagger, loot bền vững hoặc cân bằng toàn bộ boss.
...
- [ ] Rune/crystal/core chỉ thay đổi bởi kết quả mô phỏng server.
- [ ] Bốn client thấy cùng phase/HP/weakpoint và chu kỳ phối hợp hoàn thành được.
- [ ] Death/cancel/phase transition không để attack cũ tiếp tục gây damage trái luật.
...
Chơi chu kỳ đủ bốn class, phá crystal đồng thời, đánh vượt ngưỡng phase và ngắt kết nối giữa stagger; đối chiếu server log/state.
