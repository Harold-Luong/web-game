# Phase 39 — Character persistence
...
Đăng nhập lại giữ class, level, EXP, skill và tiến trình theo thiết kế.
...
**Điểm cần xử lý trước phần phụ thuộc:** Chốt một tài khoản có bao nhiêu character và chính sách phiên đồng thời trước khi triển khai phần sở hữu.

## Các bước thực hiện

1. Lưu class, level, EXP, skill/progress đã được chọn trong thiết kế, gắn ownership tài khoản.
2. Định nghĩa load/save ở ranh giới phiên/dungeon và cách xử lý hai phiên cập nhật cùng character.
3. Nối trạng thái persistent vào phiên chơi; không ghi đè progress bằng bản client tự khai.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-server/ — character/progress services`; `schema`; `client chọn/tải nhân vật`.
...
Loot/inventory/equipment, talent tree hoặc lưu realtime tick.
...
- [ ] Logout/login và restart server giữ dữ liệu character đã lưu.
- [ ] Tài khoản khác không đọc/sửa character ngoài quyền đã định.
- [ ] Cập nhật đồng thời không làm mất progress mà không được phát hiện/xử lý.
...
Tạo nhân vật, đổi tiến trình hợp lệ, đăng nhập lại và restart; thử ID của người khác và hai cập nhật cạnh tranh.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-39-character-persistence.md. Thực hiện chỉ Phase 39 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 39, chưa triển khai code.”**
