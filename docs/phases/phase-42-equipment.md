...
Slot trang bị và tác động stat rõ ràng, tránh làm hệ stat quá phức tạp.
...
**Điểm cần xử lý trước phần phụ thuộc:** Chọn slot và thời điểm được đổi đồ theo thiết kế progression; danh sách sáu slot trong nguồn là đề xuất.

## Các bước thực hiện

1. Chốt slot MVP, loại item được phép trang bị và quy tắc thay thế/tháo.
2. Tính effective stats từ base stats và trang bị, không cộng dồn vĩnh viễn lên base.
3. Đưa equip/unequip qua transaction/ownership và cập nhật phiên chơi theo thời điểm được phép.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-server/ — equipment/stat calculation`; `schema`; `client trang bị`.
...
Set bonus, enchant/crafting, item ngẫu nhiên phức tạp hoặc toàn bộ economy.
...
- [ ] Một slot không chứa hai trang bị và item không nằm ở hai nơi trái quy tắc.
- [ ] Equip/tháo nhiều lần trả stats đúng, không tích lũy bonus lỗi.
- [ ] Client không tự khai bonus; thay đồ trong dungeon tuân chính sách đã chốt.
...
Equip/unequip cùng item nhiều lần, đổi đồ nhanh/đồng thời, item sai slot và login lại; so stat với phép tính tay.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-42-equipment.md. Thực hiện chỉ Phase 42 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 42, chưa triển khai code.”**
