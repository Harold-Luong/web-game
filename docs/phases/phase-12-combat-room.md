...
Vào phòng, khóa cửa, sinh quái, dọn hết thì mở cửa.

## Đầu vào và phụ thuộc

- [Phase 11 — Enemy đầu tiên](phase-11-first-enemy.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Số enemy là cấu hình prototype, không mặc định 5 là yêu cầu cố định nếu GDD đã chọn khác.

## Các bước thực hiện

1. Tạo một combat room tĩnh với điểm vào, cửa và danh sách enemy của room.
2. Khóa cửa khi encounter bắt đầu, spawn đúng một lần và theo dõi enemy còn sống.
3. Mở cửa khi toàn bộ enemy hợp lệ đã chết; xử lý restart/fail và dọn dữ liệu encounter.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-client/src/dungeon/`; `room/door/spawn`; `scene điều phối`.
...
Procedural dungeon, checkpoint nhiều phòng, mini boss hoặc network.
...
- [ ] Không đi qua cửa khóa; cửa mở sau khi dọn hết enemy.
- [ ] Spawn/death event lặp không nhân quái hoặc mở cửa sớm.
- [ ] Chơi lại reset cửa, số enemy và listener đúng.
...
Clear phòng, chết giữa phòng rồi chơi lại, vào/ra vùng trigger nhiều lần; quan sát số enemy và trạng thái cửa.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-12-combat-room.md. Thực hiện chỉ Phase 12 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 12, chưa triển khai code.”**
