...
Client kết nối WebSocket đến Spring Boot; chưa thêm DB.

## Đầu vào và phụ thuộc

- [Phase 25 — Dungeon MVP](phase-25-dungeon-mvp.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** M4 phải có bằng chứng trước đó; thông điệp bootstrap là tạm, Phase 27 sẽ định nghĩa contract chính thức.

## Các bước thực hiện

1. Kiểm tra Java và chọn Spring Boot cùng Maven/Gradle tương thích, tạo server tối thiểu.
2. Mở kết nối WebSocket từ client, xử lý open/error/close với thông điệp kiểm tra tối thiểu.
3. Ghi cổng, cấu hình development/origin và các lệnh chạy/build thực tế; dọn kết nối khi client đóng.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-server/`; `game-client/src/network/`; `tài liệu chạy client/server`.
...
Room bốn người, game loop, full protocol, auth hoặc persistence.
...
- [ ] Browser kết nối server và nhận phản hồi kiểm tra xác định được.
- [ ] Server/client báo mất kết nối rõ khi bên kia dừng.
- [ ] Không có DB, account hoặc client tự giả lập thành server gameplay hoàn chỉnh.
...
Build server/client; mở kết nối, tắt server rồi mở lại, kiểm tra lifecycle và origin cho development.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-26-server-bootstrap.md. Thực hiện chỉ Phase 26 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 26, chưa triển khai code.”**
