...
Server quản lý mục tiêu, di chuyển, tấn công và chết của enemy.
...
**Điểm cần xử lý trước phần phụ thuộc:** Port hành vi đã kiểm chứng từ nhóm B–D; nếu khác phải ghi lý do và tác động gameplay.

## Các bước thực hiện

1. Chuyển chọn mục tiêu, aggro, movement, lịch tấn công và death của enemy lên server.
2. Tái dùng command/tick/combat; client nhận state và hiển thị, không chạy AI cạnh tranh với server.
3. Xử lý mục tiêu chết/disconnect, đổi target và despawn theo room.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-server/ — enemies/aggro/AI`; `client enemy presentation`.
...
AI mới phức tạp hơn bản offline, boss đầy đủ hoặc persistence enemy.
...
- [ ] Bốn client thấy cùng enemy ID, target, HP và hành động từ server.
- [ ] Enemy không tiếp tục đánh mục tiêu đã rời room hoặc chết ngoài luật.
- [ ] Enemy chết/despawn không còn update/timer; không ảnh hưởng room khác.
...
So snapshot bốn client, disconnect người đang bị đuổi, taunt đổi mục tiêu và clear room có nhiều enemy.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-34-server-enemy-ai.md. Thực hiện chỉ Phase 34 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 34, chưa triển khai code.”**
