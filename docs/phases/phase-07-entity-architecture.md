...
Player và enemy dùng được thành phần chung, không dồn toàn bộ logic vào scene.
...
**Điểm cần xử lý trước phần phụ thuộc:** Chỉ trừu tượng hóa phần thực tế dùng chung; dummy phục vụ kiểm chứng không phải class chơi mới.

## Các bước thực hiện

1. Định nghĩa ID, vị trí, HP, trạng thái và vòng đời entity theo nhu cầu hiện có.
2. Ghép player bằng thành phần; tạo mẫu entity/dummy tối thiểu để kiểm tra khả năng dùng chung.
3. Quy định spawn/despawn, quyền sửa state và hủy listener/timer; giữ scene ở vai trò điều phối.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-client/src/entities/`; `thành phần stats, movement, animation`.
...
ECS tổng quát, kế thừa sâu, enemy AI hoàn chỉnh hoặc backend.
...
- [ ] Player giữ nguyên movement/animation sau thay đổi.
- [ ] Dummy và player dùng chung hợp đồng HP/lifecycle mà không cần sao chép logic.
- [ ] Despawn/restart không để lại entity, timer hoặc listener còn hoạt động.
...
Kiểm tra hồi quy movement/animation và spawn/despawn nhiều lần; typecheck đảm bảo hợp đồng dùng chung.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-07-entity-architecture.md. Thực hiện chỉ Phase 7 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 7, chưa triển khai code.”**
