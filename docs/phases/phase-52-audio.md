...
Âm thanh hành động, cảnh báo boss và kết quả; phối hợp với tín hiệu thị giác.
...
**Điểm cần xử lý trước phần phụ thuộc:** Xác định nguồn/quyền sử dụng trước khi thêm file âm thanh; volume lưu ở đâu theo khả năng client hiện có.

## Các bước thực hiện

1. Chọn âm thanh hành động, boss warning, shield break, stagger, victory và nhạc nền phù hợp scope.
2. Khởi động audio qua tương tác người dùng, quản lý âm lượng/mute và chuyển scene.
3. Giới hạn âm trùng, dọn loop khi kết thúc và đồng bộ tín hiệu cảnh báo với telegraph thị giác.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `client audio manager/settings`; `asset âm thanh và nguồn sử dụng`.
...
Voice chat, sáng tác thư viện nhạc lớn hoặc mua asset ngoài yêu cầu.
...
- [ ] Game không lỗi khi browser chưa cho phát audio; người dùng có thể bật/tắt và chỉnh âm lượng.
- [ ] Cảnh báo quan trọng có cả tín hiệu thị giác, không chỉ dựa vào âm thanh.
- [ ] Restart/chuyển room không phát chồng nhạc hoặc giữ âm loop cũ.
...
Mở game lần đầu, bật/mute, đổi tab/scene và spam hit trong trận bốn người; kiểm tra audio khi boss chết.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-52-audio.md. Thực hiện chỉ Phase 52 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 52, chưa triển khai code.”**
