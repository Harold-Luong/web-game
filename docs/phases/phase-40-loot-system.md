...
Server roll thưởng và ghi nhận có kiểm soát, không cấp trùng khi xử lý lại.

## Đầu vào và phụ thuộc

- [Phase 39 — Character persistence](phase-39-character-persistence.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Phase này dùng reward ledger/entitlement tối thiểu; Phase 41 tích hợp vào inventory, phải giữ cùng định danh để tránh cấp hai lần.

## Các bước thực hiện

1. Chốt loot cá nhân/chung và điều kiện nhận; định nghĩa LootTable cùng lần trao thưởng có ID ổn định.
2. Server roll khi đủ điều kiện dungeon/boss; lưu reward grant bằng transaction và khóa chống xử lý lặp.
3. Thông báo kết quả sau khi commit; cho phép tra cứu lại phần thưởng khi client mất kết nối.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-server/ — loot/reward`; `reward ledger và migration`; `client thông báo thưởng`.
...
Inventory UI/operations đầy đủ, trading hoặc economy phức tạp.
...
- [ ] Client không tự chọn item/số lượng hoặc khai boss chết để nhận thưởng.
- [ ] Cùng lần hoàn thành được xử lý lại không cấp thêm reward.
- [ ] Lỗi DB hoặc disconnect không dẫn đến thưởng nửa chừng hay mất quyền xem reward đã ghi.
...
Gửi lại completion, chạy hai xử lý reward đồng thời và disconnect trước/sau commit; dùng random có seed trong kiểm tra để đối chiếu loot.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-40-loot-system.md. Thực hiện chỉ Phase 40 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 40, chưa triển khai code.”**
