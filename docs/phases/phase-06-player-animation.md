...
Idle/run chuyển đúng, không restart mỗi frame; thêm attack/hurt/death theo hành động.

## Đầu vào và phụ thuộc

- [Phase 05 — Player movement](phase-05-player-movement.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Chọn placeholder có thể sử dụng và ghi nguồn; chưa có asset thì nêu phương án tối thiểu trong scope thay vì tuyên bố concept sheet đã dùng được.

## Các bước thực hiện

1. Chuẩn hóa frame size, origin, hướng và khóa animation cho placeholder.
2. Đổi idle/run theo movement; thêm trạng thái attack/hurt/death qua điểm tích hợp hoặc debug khi combat chưa có.
3. Áp dụng ưu tiên animation và quy tắc loop/one-shot; đăng ký, gỡ listener theo vòng đời scene.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `game-client/src/entities/`; `thành phần animation`; `asset placeholder và thông tin nguồn`.
...
Damage, hitbox thật, art sản phẩm cuối hoặc tự tạo hàng loạt sprite.
...
- [ ] Giữ phím không làm animation chạy lại từ frame đầu mỗi update.
- [ ] Đổi hướng và dừng chuyển animation đúng, hình không nhảy do origin lệch.
- [ ] Animation one-shot kết thúc theo luật; vào lại scene không nhân listener.
...
Quan sát idle/run liên tục, đổi hướng, kích hoạt debug one-shot rồi restart scene; kiểm tra build và asset tải đúng.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-06-player-animation.md. Thực hiện chỉ Phase 6 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 6, chưa triển khai code.”**
