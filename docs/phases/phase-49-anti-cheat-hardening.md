...
Bổ sung kiểm tra/log hành vi bất thường; validation nền tảng đã phải có ở E–F.

## Đầu vào và phụ thuộc

- [Phase 32 — Authoritative movement](phase-32-authoritative-movement.md).
- [Phase 33 — Authoritative combat](phase-33-authoritative-combat.md).
- [Phase 41 — Inventory](phase-41-inventory.md).
...
**Điểm cần xử lý trước phần phụ thuộc:** Chọn ngưỡng từ tải thực tế và contract; phase này tăng cường validation vốn phải có từ E–F.

## Các bước thực hiện

1. Rà các ranh giới tin cậy movement, skill, room, inventory và reward; liệt kê validation đã có và lỗ hổng còn lại.
2. Bổ sung giới hạn tần suất/kích thước payload, replay/sequence và kiểm tra ownership phù hợp.
3. Ghi log có ngữ cảnh để điều tra, tránh đưa token/credential vào log; xử lý lỗi không làm gián đoạn room khác.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `server validation/rate limit/audit`; `tài liệu giới hạn protocol`.
...
Client obfuscation làm biện pháp chính, hệ ban tự động phức tạp hoặc hứa chống mọi cheat.
...
- [ ] Speed/cooldown/damage/loot giả bị từ chối ở server với lý do xác định.
- [ ] Spam và payload lỗi không gây tăng hàng đợi vô hạn hoặc crash handler.
- [ ] Lưu lượng chơi bình thường vẫn được chấp nhận trong điều kiện mạng đã chọn.
...
Chạy kịch bản input spam, message oversized/sai kiểu, replay, chéo room và sửa inventory; so với một phiên chơi hợp lệ.
...
> Đọc README.md, AGENTS.md và docs/phases/phase-49-anti-cheat-hardening.md. Thực hiện chỉ Phase 49 trong phạm vi đã mô tả. Kiểm tra đầu ra các phụ thuộc trước khi bắt đầu; không tự làm phase khác để lấp phần thiếu. Hoàn thành các bước được giao, kiểm chứng checklist, cập nhật báo cáo trong file phase và nêu rõ phần còn thiếu. Giữ các đề xuất chưa chốt ở trạng thái đề xuất.

Nếu chỉ muốn tiếp tục review tài liệu, thêm vào yêu cầu: **“Chỉ rà soát/cập nhật tài liệu Phase 49, chưa triển khai code.”**
