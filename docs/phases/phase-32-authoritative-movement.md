# Phase 32 — Authoritative movement
...
Server xác định vị trí từ input; client hiển thị prediction/reconciliation theo thiết kế.
...
**Điểm cần xử lý trước phần phụ thuộc:** Hợp nhất đơn vị và hình học collision trong protocol; không dùng frame animation làm đồng hồ di chuyển server.

## Các bước thực hiện

1. Server nhận hướng/input và tự tính vị trí, tốc độ, world bounds/collision theo luật client offline.
2. Client dự đoán chuyển động cục bộ, nhận authoritative state và đối chiếu sequence đã xử lý.
3. Reconcile input còn chờ; remote player tiếp tục interpolation, reset prediction khi respawn/chuyển room.

## Đầu ra và phạm vi file

- Đường dẫn/thành phần dự kiến: `server movement/collision`; `client prediction/reconciliation`; `protocol movement ack`.
...
Combat authoritative, lag compensation nâng cao hoặc rollback toàn game.
...
- [ ] Gửi tọa độ/tốc độ giả không thay vị trí chính thức ngoài luật server.
- [ ] Cùng input theo thời gian tạo movement đúng giới hạn và tốc độ chéo.
- [ ] Client hội tụ về state server sau trễ/chỉnh sai mà không replay input đã ack.
...
Thử input spam, tốc độ giả, giữ phím khi disconnect, rìa map và latency/jitter; so vị trí server với bốn client.
